...
* therefore it is important not just to put "session_start();" at the top of every page on which you want to use PHP sessions.
...
* Also the "session_regenerate_id()" function, which regenerates the session id on every page reload,
* helps prevent session hijacking. Note: If you are using HTTPS in your login application set the "$secure" variable to true.
...
/*
* In our code we'll log failed attempts and lock the user's account after five failed login attempts.
* This should trigger the sending of an email to the user with a reset link,
* but we have not implemented this in our code.
...
* To check if they are equal, we use the hash_equals function to prevent timing attacks.
